...
class AllHosesNot extends StatefulWidget {
  const AllHosesNot({Key? key}) : super(key: key);

  @override
  State<AllHosesNot> createState() => _AllHosesNotState();
}

class _AllHosesNotState extends State<AllHosesNot> {
  ...
  Future openBox() async{
    ...
    Hive.init(dir.path);
    box=await Hive.openBox('all not');
    return;
  }
  ...
  Future   getHouses()async {
    ...
    try{
      ...
      var response = await http.get(
          Uri.parse("https://www.eazyrent256.com/api/user/all/all.php"),
    ...
    if (snapshot.hasData) {
      ...
      if(document==0){
        ...
        return  Center(child: Column(
          ...
          children: const [
            Text('No Places',style: TextStyle(fontSize: 17,fontWeight: FontWeight.bold),),
          ],
        ));
      }
